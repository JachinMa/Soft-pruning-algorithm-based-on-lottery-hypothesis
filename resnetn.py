...
class CIFAR_ResNet(nn.Module):
    def __init__(self, block=BasicBlock, num_blocks=[2,2,2,2], num_classes=100, bias=True):
    ...
    def prunable_block_names(self,block):
        ...
        return [name + '.weight' for name, module in self.named_modules() if
                (isinstance(module, torch.nn.modules.conv.Conv2d) and (name[:6] == 'layer' + str(block)))]
...
def cifarresnet18(pretrained=False, **kwargs):
    return CIFAR_ResNet(BasicBlock, [2,2,2,2])
    
def cifarresnet18_tiny(pretrained=False, **kwargs):
    return CIFAR_ResNet(BasicBlock, num_blocks=[2,2,2,2],num_classes=200)
    
def cifarresnet56(pretrained=False, **kwargs):
    return CIFAR_ResNet(BasicBlock, [9,9,9])

def cifarresnet50(pretrained=False, **kwargs):
    return CIFAR_ResNet(BasicBlock, num_blocks=[3,4,6,3],num_classes=100)
